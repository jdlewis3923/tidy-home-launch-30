...
Deno.serve(async (req) => {
  ...
  try {
    ...
    // Catch-all: always return CORS headers so failures surface as readable JSON
    // instead of a bare runtime 500 (which supabase-js reports as a network error).
    const message = err instanceof Error ? err.message : String(err);
    console.error('[sync-addon-stripe-catalog] unhandled', message);
    return jsonResponse({ ok: false, error: 'internal_error', detail: message }, 500);
  }
});
