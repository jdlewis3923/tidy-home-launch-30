/**
 * PRICING CANON — the ONE source of truth for every price, size and promise.
 *
 * The model: three sizes (1, 2, 3) on every service.
 *  - House Cleaning is priced PER VISIT, by bedrooms.
 *  - Lawn Care is priced PER VISIT, by mowable turf.
 *  - Shine Complete is priced PER MONTH, by vehicle.
 *  - The Car Wash Add-On is priced PER MONTH, by vehicle and wash count.
 *
 * Cadence is carried by the Stripe subscription QUANTITY (monthly 1, biweekly 2,
 * weekly 4) and only applies to per-visit services. Shine Complete and the Car
 * Wash Add-On are always quantity 1.
 *
 * Stripe is referenced by lookup_key, never by price ID.
 *
 * The mirror used by edge functions lives at
 * `supabase/functions/_shared/pricing-canon.ts`; `src/test/pricing-canon.test.ts`
 * fails if the two diverge or if a page ships a stale number.
 */
