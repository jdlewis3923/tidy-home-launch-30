...
export default function Apply() {
  ...
  const submit = async (e: React.FormEvent) => {
    e.preventDefault();
    if (!form.service)        { toast({ title: t("Please pick a role"),            variant: "destructive" }); return; }
    if (!form.experience)     { toast({ title: t("Please pick experience range"),  variant: "destructive" }); return; }
    if (!form.has_vehicle)    { toast({ title: t("Reliable transportation?"),      variant: "destructive" }); return; }
    if (!form.has_supplies)   { toast({ title: t("Professional equipment?"),       variant: "destructive" }); return; }
    if (!form.work_authorized){ toast({ title: t("US work authorization?"),        variant: "destructive" }); return; }
    ...
    try {
      ...
      console.error(err);
      toast({ title: t("Could not submit"), description: err?.message ?? t("Please try again"), variant: "destructive" });
    } finally {
      setSubmitting(false);
    }
  };
  ...
  if (done) {
    return (
      <main className="min-h-screen bg-navy-deep relative overflow-hidden flex items-center justify-center p-6">
        <Helmet><title>{t("Application received | Tidy")}</title></Helmet>
        ...
        <div className="relative max-w-lg w-full rounded-2xl bg-white/[0.04] backdrop-blur-xl border border-white/10 p-10 text-center shadow-2xl animate-calm-rise">
          ...
          <h1 className="mt-6 font-display text-3xl font-black text-white tracking-tight">{t("Application received")}</h1>
          <p className="mt-3 text-white/70 leading-relaxed">
            {t("Thanks for applying. We'll review and be in touch within")} <span className="text-white font-semibold">2–3 business days</span>.
          </p>
  ...
  return (
    <main className="min-h-screen bg-navy-deep relative overflow-hidden">
      <Helmet>
        ...
        <meta name="description" content={t("Join Tidy's contractor network in Kendall and Pinecrest. Cleaning, lawn care, and car detailing pros — weekly pay, predictable routes.")} />
      </Helmet>
      ...
      <div className="relative z-10 max-w-7xl mx-auto px-4 sm:px-6 pb-20 pt-6 grid lg:grid-cols-[1.05fr_1fr] gap-10 lg:gap-16 items-start">
        <section className="text-white animate-calm-rise">
          ...
          <p className="mt-5 max-w-xl text-base sm:text-lg text-white/70 leading-relaxed">
            {t("Tidy is Miami's subscription home-service brand. We're hiring vetted cleaners, lawn pros, and detailers in Kendall and Pinecrest.")}
          </p>
          ...
          <div className="mt-10 flex flex-wrap items-center gap-x-6 gap-y-3 text-sm text-white/55">
            ...
            <span className="inline-flex items-center gap-1.5"><ShieldCheck className="h-4 w-4 text-gold" /> {t("Background check on every hire")}</span>
          </div>
        </section>
        ...
        <section className="relative">
          ...
          <div className="relative rounded-2xl bg-white shadow-2xl border border-white/40 overflow-hidden animate-calm-rise">
            <div className="px-6 sm:px-8 pt-7 pb-5 border-b border-hairline bg-cream">
              <h2 className="mt-1 font-display text-2xl font-black text-ink tracking-tight">{t("Apply to join Tidy")}</h2>
            ...
            <form onSubmit={submit} className="px-6 sm:px-8 py-7 space-y-5">
              <div className="grid grid-cols-1 sm:grid-cols-2 gap-4">
                <div>
                  <Label htmlFor="first_name" className="text-ink">{t("First name")} *</Label>
                  <Input id="first_name" required value={form.first_name} onChange={(e) => set("first_name", e.target.value)} className="mt-1.5" />
                </div>
                <div>
                  <Label htmlFor="last_name" className="text-ink">{t("Last name")} *</Label>
              ...
              <YesNoBlock label={t("Do you have your own reliable transportation?") + " *"} value={form.has_vehicle}     onChange={(v) => set("has_vehicle", v)}     name="vehicle" />
              <YesNoBlock label={t("Do you have your own professional equipment in working condition?") + " *"} value={form.has_supplies} onChange={(v) => set("has_supplies", v)} name="supplies" />
              <YesNoBlock label={t("Are you authorized to work in the United States?") + " *"} value={form.work_authorized} onChange={(v) => set("work_authorized", v)} name="workauth" />

              <div>
                <Label htmlFor="description" className="text-ink">{t("Brief description of your relevant experience")}</Label>
                <Textarea id="description" rows={4} maxLength={500} value={form.description} onChange={(e) => set("description", e.target.value)} className="mt-1.5" placeholder={t("Tell us about your background in this service…")} />
              ...
              <div className="rounded-xl border border-hairline bg-cream/60 p-4 text-xs text-ink leading-relaxed">
                {t("By submitting, you confirm Tidy may contact you about this role. You will undergo a background check at Tidy's expense if we move forward.")}
              </div>
