...
import { withTrackedParams } from "@/lib/landing";
...
const Neighbor = () => {
  const { t } = useLanguage();
  const signupHref = withTrackedParams("/signup");
          ...
          <div className="max-w-3xl mx-auto text-center">
            <Reveal>
              ...
              <div className="mt-8 grid gap-4 md:grid-cols-3 text-left">
                {(["cleaning", "lawn", "shine"] as const).map((service, i) => {
                  const p = getServiceStartingPrice(service);
                  return (
                    <div key={service} className="rounded-xl border bg-card p-5" data-index={i}>
                      <p className="text-sm font-semibold text-foreground">{t(p.label)}</p>
                      <p className="text-2xl font-bold text-foreground mt-2 tabular-nums">{p.display}</p>
                      <p className="text-xs text-text-mid mt-1">{t(p.helper)}</p>
                    </div>
                  );
                })}
              </div>
