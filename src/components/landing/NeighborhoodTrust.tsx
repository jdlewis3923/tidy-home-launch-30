    ...
    <div className="max-w-4xl mx-auto">
      <Reveal className="text-center mb-8">
        <h2 className="text-2xl md:text-3xl font-bold text-foreground">
          Built for Pinecrest + Kendall.
        </h2>
        <p className="text-sm md:text-base text-text-mid mt-3 max-w-2xl mx-auto leading-relaxed">
          We serve only 33156, 33183, and 33186 — so your crew is local, on-time,
          and never stuck in traffic.
        </p>
      </Reveal>
        ...
        {ZIPS.map((z, i) => (
          <Reveal key={z.zip} delay={i * 80}>
            <div className="bg-card border rounded-xl p-4 md:p-6 text-center hover-lift">
              ...
              <p className="text-xs md:text-sm text-text-mid mt-1">{z.name}</p>
            </div>
          </Reveal>
        ))}
      </div>
