      ...
      <div className="relative z-10 max-w-2xl mx-auto text-center">
        ...
        <div className="flex gap-3 max-w-md mx-auto mb-6">
          <input
            ...
            className="flex-1 bg-navy-deep border border-primary-foreground/20 text-primary-foreground rounded-lg px-4 py-3 text-sm placeholder:text-primary-foreground/30 focus:outline-none focus:ring-2 focus:ring-primary"
            onKeyDown={(e) => e.key === "Enter" && handleCheck()}
          />
          <button onClick={handleCheck} className="bg-gold hover:bg-gold/90 text-gold-foreground font-semibold px-6 py-3 rounded-lg text-sm transition-colors">
            Check →
          </button>
        </div>

        {result && (
          <p className={`text-sm font-medium mb-6 ${result.found ? "text-success" : "text-destructive"}`}>
            {result.found
